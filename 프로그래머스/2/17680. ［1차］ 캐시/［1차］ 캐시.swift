...
// 문제 오독으로 인한 오답 - 캐시 삭제 기준을 (최저 실행 횟수 + 가장 오래된 캐시) 라고 이해함
// 배열을 2개 사용 - 실행 횟수 / 도시 이름
func solution(_ cacheSize:Int, _ cities:[String]) -> Int {
    // LRU 캐시: 가장 오랫동안 사용되지 않은 캐시를 삭제
    ...
    for e in cities {
        ...
        if cacheCity.contains(city) { // cache hit
            ...
            if cacheCity.count >= cacheSize { // 빈 캐시가 없을 경우
                let minCount = cacheCount.min()! // 캐시 중 가장 적은 실행 횟수
                let i = cacheCount.firstIndex(of: minCount)! // minCount 중 가장 실행이 오래된 캐시
                
                // 캐시 삭제
                cacheCity.remove(at: i)
                cacheCount.remove(at: i)
            }
