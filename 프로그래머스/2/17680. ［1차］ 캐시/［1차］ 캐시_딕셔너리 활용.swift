...
// --> 왜 아래의 풀이가 정답 처리 되었는가?: for문 내부의 else문에서, cache.sorted를 할 때 count 기준으로 정렬한 후 index 기준으로 다시 정렬한다.
// --> 결국 index 기준으로 정렬하기 때문에 가장 실행이 오래된 순서로 정렬될 것이라 사실 첫 정렬은 의미 없어지고 실행 순서에 따라 정렬된다. 그래서 문제 요구사항에 맞게 정답이 도출되었다.
// --> 즉, 정답은 맞췄지만 여러모로 안좋은 코드라 생각된다.
